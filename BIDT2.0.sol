...
contract BIDTTokenTest2 is StandardToken {
	...
	uint8 public  basePublicPlacement  = 1;
	...
	function getRate() constant returns() {
		return baseRate;
	}
	
	function setPublicPlacementNum(uint publicPlacement) public onlyOwner {
		basePublicPlacement = basePublicPlacement;
	}
	function getPublicPlacementNum() constant returns() {
		return baseRate;
	}
	...
	function getBuyState() constant returns() {
		return allowedBuy;
	}
	...
	function BIDTTokenTest2(address _target) {
		target = _target;
	...
	function computeTokenAmount(uint ethAmount) internal constant returns(uint tokens) {
		...
		if(	balances[target] > balances[target].div(10).mul(8-basePublicPlacement)){
		    	tokens = tokenBase;
		}
	}
